...
print(f"File : {f.name}")
print(
    f"Config: {cfg.get('embed_models', [])} x {cfg.get('llm_models', [])} x {cfg.get('search_modes', [])}"
)
print(f"Cells : {len(results)} / {d.get('config', {}).get('total_cells', '?')}")
print()
print(
    f"{'Embed':<22} {'LLM':<16} {'Mode':<20} {'P@K':>6} {'MRR':>6} {'NDCG@K':>8} {'Faith':>6} {'AR':>6} {'CP':>6} {'CR':>6} {'Gnd':>6}"
)
print("-" * 120)
for r in results:
    ...
    print(
        f"{coll:<22} {llm:<16} {mode:<20} {fmt(p5)} {fmt(mrr)} {fmt(ndcg)} {fmt(faith)} {fmt(ar)} {fmt(cp)} {fmt(cr)} {fmt(gnd)}{err}"
    )
