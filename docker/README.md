...
![RAGEve](https://raw.githubusercontent.com/bazzi24/RAGEve/main/docs/assets/banner.png)
...
##  Demo
<div align="center">
  <img src="https://raw.githubusercontent.com/bazzi24/RAGEve/main/docs/assets/demo.gif" alt="RAGEve" />
</div>
...
RAGEve uses environment variables for configuration. There are two scenarios:

**For Docker deployments** (using `docker compose -f docker/docker-compose.yml`):
...
The `.env` file must be located in the `docker/` directory because `docker-compose.yml` references it with a relative path.

**For local development** (running backend directly with `uvicorn`):
...
When running the backend outside Docker, `config.py` loads `.env` from the current working directory (project root).

---

#### Environment Variables

All configuration is centralized in `docker/.env` (or project root `.env` for local dev). The following variables are available:

**Core Database Settings**

| Variable | Default | Description |
|---|---|---|
| `MYSQL_HOST` | `mysql` (Docker) / `localhost` (local) | MySQL server hostname (Peewee ORM — primary) |
| `MYSQL_PORT` | `3306` | MySQL server port |
| `MYSQL_USER` | `root` | MySQL username |
| `MYSQL_PASSWORD` | _(configured via MYSQL_ROOT_PASSWORD)_ | MySQL password |
| `MYSQL_DBNAME` | `rag_eve` | MySQL database name (27-table schema) |
| `DB_URL` | _(auto-generated for Docker)_ | **Legacy:** SQLAlchemy DSN for chat history (e.g., `mysql+aiomysql://...` or `sqlite:///data/chat.db`). Auto-configured in Docker; override only for custom setups. |

**Service URLs**

| Variable | Default | Description |
|---|---|---|
| `OLLAMA_BASE_URL` | `http://host.docker.internal:11434` (Docker) / `http://localhost:11434` (local) | Ollama server URL |
| `QDRANT_URL` | `http://qdrant:6333` (Docker) / `http://localhost:6333` (local) | Qdrant server URL |
| `QDRANT_API_KEY` | _(none)_ | Qdrant API key when auth is enabled |
| `VECTOR_DB_PROVIDER` | `qdrant` | Vector database provider (currently only `qdrant` supported) |
...
| Variable | Default | Description |
|---|---|---|
| `DATA_ROOT` | `/app/data` (Docker) / `data` (local) | Base directory for uploads, chunks, vectors, logs |
| `UPLOAD_DIR_NAME` | `uploads` | Subdirectory for uploaded files (relative to DATA_ROOT) |
| `CHUNK_DIR_NAME` | `chunks` | Subdirectory for extracted text chunks |
| `VECTOR_DIR_NAME` | `vector` | Subdirectory for vector index data |
| `DEFAULT_CHUNK_SIZE` | `1200` | Default chunk size in characters |
| `DEFAULT_CHUNK_OVERLAP` | `180` | Overlap between chunks in characters |
| `DEFAULT_MAX_TOKENS_PER_CHUNK` | `500` | Maximum tokens per chunk (for LLM context management) |
...
| `OCR_ENGINE` | `paddle` | OCR engine: `paddle` (recommended) or `tesseract` |
| `OCR_THRESHOLD_CHARS` | `50` | Minimum chars extracted to consider PDF not scanned (skip OCR if above) |
| `COLUMN_HISTOGRAM_BINS` | `50` | Number of bins for histogram-based column detection |
| `COLUMN_PEAK_THRESHOLD` | `0.3` | Minimum peak height ratio (0-1) for column detection |
| `COLUMN_MIN_GAP` | `20.0` | Minimum gap (points) between columns |
| `PDFPLUMBER_TABLE_STRATEGY` | `text` | Table extraction: `text`, `lines`, or `explicit` |
| `PDFPLUMBER_SNAP_TOLERANCE` | `3.0` | Snap tolerance for pdfplumber (points) |
| `PDFPLUMBER_JOIN_TOLERANCE` | `3.0` | Join tolerance for pdfplumber (points) |

**Upload Limits**

| Variable | Default | Description |
|---|---|---|
| `MAX_UPLOAD_BYTES` | `524288000` (500 MB) | Maximum file size per upload |
| `MAX_DATASET_BYTES` | `107374182400` (100 GB) | Maximum total dataset size per knowledge base |
