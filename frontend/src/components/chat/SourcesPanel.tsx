...
export function SourcesPanel({ sources, rerankerModel }: SourcesPanelProps) {
  ...
  const canPreview = (s: SourceChunk) => {
    return !!(s.blocks && s.blocks.length > 0 && s.datasetId && s.source);
  };
    ...
    <div className={styles.panel}>
      <div className={styles.header}>
        ...
        <div className={styles.headerRight}>
          {rerankerModel && (
            <span
              className={styles.rerankerBadge}
              title={`Reranked with ${rerankerModel}`}
            >
      ...
      {sources.length === 0 ? (
        ...
        <div className={styles.list}>
          {sources.map((s, i) => (
            <div
              key={String(s.chunk_id ?? `source-${i}`)}
              ...
              <div className={styles.sourceMeta}>
                <div className={styles.sourceHeaderLeft}>
                  ...
                  {s.search_type === "hybrid" && (
                    <Badge
                      variant="info"
                      title="Retrieved by hybrid search (dense + sparse)"
                    >
                      HYBRID
                    </Badge>
                  )}
                ...
                <div className={styles.scoreRow}>
                  {rerankerModel && s.cosine_score != null ? (
                    <>
                      <Badge
                        variant="muted"
                        title="Bi-encoder cosine similarity"
                      >
